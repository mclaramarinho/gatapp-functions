...
export const getPet = onRequest(async (req, res) => {
    try {
        ...
        const petDoc = await firestore.collection(FirestoreCollections.Pets).doc(petId).get();
        if(!petDoc.exists) {
        ...
        if(petData?.ownerId !== ownerIdFromToken) {
            res.status(403).json({ message: "You do not have permission to access this pet" });
            return;
        }
...
export const getPetsByOwner = onRequest(async (req, res) => {
    try {
        ...
        const petsSnapshot = await firestore.collection(FirestoreCollections.Pets)
