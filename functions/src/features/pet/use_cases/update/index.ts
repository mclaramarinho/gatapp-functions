...
export const updatePet = onRequest(async (req, res) => {
    try {
        ...
        const petDoc = await firestore.collection(FirestoreCollections.Pets).doc(petId as string).get();
        if(!petDoc.exists) {
            res.status(404).send({ error: "Pet not found" });
            return;
        } else if(petDoc.data()?.ownerId !== ownerId) {
            res.status(403).send({ error: "You do not have permission to update this pet" });
            return;
        }

        const model = new WritePetModel(new Map(Object.entries(req.body)));
        await firestore.collection(FirestoreCollections.Pets).doc(petId as string).update({
            ...model
        });
